...
import { listarCierres } from "../apiUrls";
...
import ModalCirreForm from "../modals/cierre/modalCierre";

const TablaCierres = (props) => {
    const { listaCierres, setListaCierres, caja } = useContextCaja();
    ...
    const CrearTabla = () => {
        let table = new DataTable('#tablaCierres', {
            data: listaCierres,
            "columnDefs": [
                {
                    "targets": 0,
                ...
                },
                {
                    ...
                    createdCell: (td, cellData, rowData, row, col) => {
                        if (rowData) {
                            const root = createRoot(td);
                            root.render(
                                <ModalCirreForm key={rowData.id} accion={false} datosCierre={rowData} datosCaja={caja} ActualizarTabla={ActualizarTabla} datosUsuario={usuario}></ModalCirreForm>
                            );
                        }
            ...
            drawCallback: function () {
                $('.dataTables_filter').find("[aria-controls='tablaCierres']").addClass('form-control');
    ...
    return (
        <div>
            <table id="tablaCierres" className="table table-bordered table-hover">
            </table>
        </div>
    );
}
